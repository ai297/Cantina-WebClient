<template>
    <div class="sndMForm">
        <div class="inputUserListMenu">
            <pop-up-users v-if="selectedUserMode" :list="sortedUsersList" :top="popUpUsersCoord.y" :left="popUpUsersCoord.x" :selected="popUpSelectedIndex" />
        </div>
...
export default {
    ...
    computed: {
        ...
        sortedUsersList: function() {
            ...
            for(let user of this.usersInOnline) {
                if(this.isMessageStringContainsUsername(user.name) || this.sortValue.length > user.name.length || user.userId == this.currentUser.id) continue;
    ...
    methods: {
        ...
        addNicknameToMessageString: function(payload) {
            ...
            this.selectedUserMode = false; // закрывает менюшку с выбором юзеров
        },
        ...
        inputKeyDown: function(key) {
            switch(key) {
                case "ArrowDown":
                    if(this.selectedUserMode && this.popUpSelectedIndex < this.sortedUsersList.length - 1) this.popUpSelectedIndex++;
                    break;
                case "ArrowUp":
                    if(this.selectedUserMode && this.popUpSelectedIndex > 0) this.popUpSelectedIndex--;
                    break;
                case "Enter":
                    if(this.selectedUserMode) this.addNicknameToMessageString({messageType: MESSAGE_TYPES.Base, userName: this.sortedUsersList[this.popUpSelectedIndex]});
                    else this.submit();
                    break;
                case "Tab":
                    if(this.selectedUserMode) {
        ...
        // Отправка сообщения
        submit: function(){
            if(this.selectedUserMode) return;
    ...
    watch: {
        sortedUsersList: function(val) {
            ...
            if(val.length == 0) this.selectedUserMode = false;
        }
    },
    mounted: function(){
