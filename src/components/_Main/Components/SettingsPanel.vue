<template>
    <div class="chatSettingsPanel" :style="{'flex-direction': (reverseDirection && !isMinWidth()) ? 'row-reverse' : 'row'}">
        ...
        <volume-button v-if="!isMinWidth()" @click="changeWidth" :active="!isLimitedChatWidth" title="Растянуть на весь экран (для экранов FullHD и больше)"><cantina-icons iconName="maximize" /></volume-button>
        <volume-button v-if="!isMinWidth()" @click="changeAsideBlockMode" :active="!isShowSidebar" title="Скрывать боковую панель (будет показываться при наведении курсора в правую часть экрана)"><cantina-icons iconName="aside" /></volume-button>
        ...
        <volume-button v-if="!isMinWidth()" @click="switchDirection" :active="reverseDirection" title="Переместить боковую панель влево"><cantina-icons iconName="direction" /></volume-button>
    </div>
</template>
